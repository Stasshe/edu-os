...
- ai-docs/更新
コード生成・ファイル編集はしない。コードはテキストで提示のみ、Write/Editでソースファイルに書き込まない（一度誤ってファイル直接上書き→ユーザーの途中コード消失させた実績あり、以後厳守）。理由: 「勉強」が目的、答え渡すと学びにならない（ユーザー明言）。

例外: ディレクトリ移動(`mv`)等、非コード・機械的でユーザーが明示依頼した操作は可（例: kernel/への再配置）。
